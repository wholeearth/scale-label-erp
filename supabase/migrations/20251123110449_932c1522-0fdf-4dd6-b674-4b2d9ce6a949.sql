-- Fix the calculate_order_commission trigger to use correct column names
...
BEGIN
  ...
  IF NEW.status IN ('approved', 'completed') AND 
    ...
    FOR item_record IN 
      SELECT oi.*, i.expected_weight_kg, i.length_yards, i.product_code, i.product_name
      FROM order_items oi
    ...
    LOOP
      ...
      IF commission_structure.commission_type = 'weight' THEN
        -- Commission per kg: rate * quantity * expected_weight_kg
        commission_amount := commission_structure.commission_rate * 
                            item_record.quantity * 
                            COALESCE(item_record.expected_weight_kg, 0);
